...
public class ImdbServer extends AbstractServer {
    private static final String IMAGE_BASE_URL = "https://image.tmdb.org/t/p/w500";
    private static final String LANGUAGE = "ar-EG";

    static String TAG = "Imdb";
    public static String WEBSITE_URL = "https://api.themoviedb.org/3/";

    private static final String API_KEY = "0ba8a205453bbcae968348bcb9a2976b"; // Replace with your TMDB API key

    public ImdbServer() {
    }

    private void fetchPopularMovies(ActivityCallback<ArrayList<Movie>> activityCallback) {
        TmdbApi api = RetrofitClient.getApi();
        Call<ImdbMovieResponse> call = api.getPopularMovies(API_KEY, LANGUAGE);

        call.enqueue(new Callback<ImdbMovieResponse>() {
            @Override
            public void onResponse(Call<ImdbMovieResponse> call, Response<ImdbMovieResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    activityCallback.onSuccess(generateMovieList(response.body().getResults()), getLabel());
                } else {

                    Log.d(TAG, "Failed to load movies: ");
                }
            }

            @Override
            public void onFailure(Call<ImdbMovieResponse> call, Throwable t) {
                Log.d(TAG, "Failed to load movies: "+t.getMessage());
            }
        });
    }

    private ArrayList<Movie> generateMovieList(ArrayList<ImdbMovieDto> results) {
        ArrayList<Movie> movieList = new ArrayList<>();
        for (ImdbMovieDto result : results) {
            Movie movie = new Movie();
            movie.setTitle(result.title);
            movie.setDescription(result.overview);
            movie.setCardImageUrl(IMAGE_BASE_URL +result.poster_path);
            movie.setBackgroundImageUrl(result.poster_path);
            movie.setStudio(Movie.SERVER_IMDB);
            movie.setState(Movie.ITEM_STATE);

            // Assuming result.vote_average is a float or double
            double voteAverage = result.vote_average; // e.g., 3.459

            String formattedRate = String.format("%.1f", voteAverage);
            movie.setRate(formattedRate); // formattedRate will be "3.5" (due to rounding) or "3.4" if you truncate

// To strictly truncate to one decimal place (like "3.4" from 3.459)
// you can do a little math before formatting:
            double truncatedVoteAverage = Math.floor(voteAverage * 10) / 10.0;
            String formattedTruncatedRate = String.format("%.1f", truncatedVoteAverage);
            movie.setRate(formattedTruncatedRate); // formattedTruncatedRate will be "3.4"

//            movie.setVideoUrl(result.getVideoUrl());
            Log.d(TAG, "Movie: " + result);
            movieList.add(movie);
        }
        return movieList;
    }
    ...
    private void fetchMovieDetails(int movieId) {
        TmdbApi api = RetrofitClient.getApi();
        Call<ImdbMovieDto> call = api.getMovieDetails(movieId, API_KEY, LANGUAGE);

        call.enqueue(new Callback<ImdbMovieDto>() {
            @Override
            public void onResponse(Call<ImdbMovieDto> call, Response<ImdbMovieDto> response) {
...
//                    Toast.makeText(MovieDetailActivity.this, "Failed to load movie details", Toast.LENGTH_SHORT).show();
            ...
            @Override
            public void onFailure(Call<ImdbMovieDto> call, Throwable t) {
//                progressBar.setVisibility(View.GONE);
//                Toast.makeText(MovieDetailActivity.this, "Error: " + t.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    public ArrayList<Movie> search(String query, ActivityCallback<ArrayList<Movie>> activityCallback) {
        ...
        fetchPopularMovies(activityCallback);
        return null;
